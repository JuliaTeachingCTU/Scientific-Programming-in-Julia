...
Can be written in latex: ``\\alpha = 1``
"""
addπ(x) = x+π
```
...
By default every package is precompiled when loading.

If it defines methods that extends previously defined (e.g. from Base), it may affect already loaded packages which need to be recompiled as well.
